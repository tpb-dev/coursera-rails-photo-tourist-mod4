(function() {
  ...
  ImageEditorController.$inject = ["$scope","$q",
                                  ...
                                  "spa-demo.subjects.ImageTrip",
                                  "spa-demo.subjects.ImageLinkableTrip",
                                   "spa-demo.geoloc.geocoder",
                                   ];
  function ImageEditorController($scope, $q, $state, $stateParams, 
                                 Authz, DataUtils, Image, ImageThing,ImageLinkableThing, ImageTrip, ImageLinkableTrip,
                                 geocoder) {
    var vm=this;
    ...
    function reload(imageId) {
      ...
      vm.trips = ImageTrip.query({image_id:itemId});
      vm.linkable_things = ImageLinkableThing.query({image_id:itemId});
      vm.linkable_trips = ImageLinkableTrip.query({image_id:itemId});
      vm.imagesAuthz.newItem(vm.item);
      $q.all([vm.item.$promise,
              vm.things.$promise, vm.trips.$promise]).catch(handleError);
    ...
    function linkThings(parentPromise) {
      ...
      function linkTrips(parentPromise) {
          var promises=[];
          if (parentPromise) { promises.push(parentPromise); }
          angular.forEach(vm.selected_linkables, function(linkable){
              var resource=ImageTrip.save({image_id:vm.item.id}, {trip_id:linkable});
              promises.push(resource.$promise);
          });
